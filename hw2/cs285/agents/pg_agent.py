...
class PGAgent(nn.Module):
    ...
    def update(
        ...
        if self.critic is not None:
            ...
            for _ in range(self.baseline_gradient_steps):
                ...
                critic_info.update(critic_update_info)
    ...
    def _calculate_q_vals(self, rewards: Sequence[np.ndarray]) -> Sequence[np.ndarray]:
        ...
        for reward in rewards:
            ...
            q_values_array.append(np.array(q_values, dtype = np.float32))
    ...
    def _estimate_advantage(
        ...
        if self.critic is None:
            ...
            if self.gae_lambda is None:
                ...
                discount_factor = self.gae_lambda*self.gamma
                for i in reversed(range(batch_size)):
                    ...
                    if terminals[i] == 1:
                        ...
                        current_a = rewards[i] + self.gamma*values[i+1] - values[i]
                        advantages[i] = current_a + discount_factor*advantages[i+1]
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        involve t)!
        """
        total_discounted_return = sum(reward * self.gamma**t for t, reward in enumerate(rewards) )
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for i in range(2, T+1):
            reward_to_go[-i] = self.gamma*reward_to_go[-(i-1)] + rewards[-i]
