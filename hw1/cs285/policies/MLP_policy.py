...
class MLPPolicySL(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def forward(self, observation: torch.FloatTensor) -> Any:
        ...
        raw_actions = torch.normal(mean, std)

        # ensure lies between -1 and 1 to qualify for action space
        actions = torch.tanh(raw_actions)

        return actions

    def update(self, observations: torch.FloatTensor, actions: torch.FloatTensor):
        """
